...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        do {
            try CurrentEnvironment.mealStore.load()
        } catch {
    ...
    var shouldShowWelcome: Bool {
        ...
        #endif
        return CurrentEnvironment.mealStore.meals.isEmpty
    }
    ...
    func sceneWillResignActive(_ scene: UIScene) {
        ...
        do {
            try CurrentEnvironment.mealStore.save()
        } catch {
...
extension SceneDelegate: WalkthroughDelegate {
    func didCompleteWalkthrough(firstDinner: String?) {
        ...
        if let firstDinner = firstDinner {
            CurrentEnvironment.mealStore.add(Meal(name: firstDinner, date: Date(), type: .dinner))
        }
